...
// ---------- Single source of truth for font + colors — same shape as Header.jsx / Contact.jsx / Projects.jsx / Experience.jsx ----------
...
const CATEGORY_ICONS = [
  ...
  (props) => (
    <svg {...props} width="18" height="18" viewBox="0 0 24 24" fill="none">
      <path d="M12 6c-1.5-1.3-3.6-2-6-2-1 0-2 .1-3 .4v13c1-.3 2-.4 3-.4 2.4 0 4.5.7 6 2 1.5-1.3 3.6-2 6-2 1 0 2 .1 3 .4v-13c-1-.3-2-.4-3-.4-2.4 0-4.5.7-6 2Z" stroke="currentColor" strokeWidth="2" strokeLinejoin="round" />
  ...
  (props) => (
    <svg {...props} width="18" height="18" viewBox="0 0 24 24" fill="none">
      ...
      <path d="M9 14.5 8 21l4-2 4 2-1-6.5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  ),
];
...
function EducationCard({ education, onOpenDetails }) {
  return (
    <div
      role="button"
      tabIndex={0}
      onClick={() => onOpenDetails(education)}
      onKeyDown={(e) => {
        if (e.key === "Enter" || e.key === " ") onOpenDetails(education);
      }}
    ...
    >
      <div className="w-full h-44 md:h-48 overflow-hidden" style={{ backgroundColor: THEME.textDark }}>
        <img src={education.image} alt={education.institutionName} className="w-full h-full object-cover" loading="lazy" />
      </div>
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div
          ...
          onClick={(e) => e.stopPropagation()}
        >
          <IconAction label={`About ${education.institutionName}`} onClick={() => onOpenDetails(education)}>
            <InfoIcon />
          </IconAction>

          {education.scoreCardUrl && (
            <IconAction label={`View ${education.institutionName} score card`} href={education.scoreCardUrl}>
              <LinkIcon />
            </IconAction>
          )}
        </div>
...
function EducationGrid({ educations, onOpenDetails }) {
  return (
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
      {educations.map((education) => (
        <EducationCard key={education.id} education={education} onOpenDetails={onOpenDetails} />
      ))}
    </div>
  );
}
...
function CategorySection({ label, educations, isExpanded, onToggle, onOpenDetails, iconIndex }) {
  const count = educations?.length ?? 0;
    ...
    >
      ...
      >
        <div className="px-5 pb-6 pt-2 md:px-6 md:pb-7" style={{ borderTop: `1px solid ${THEME.border}` }}>
          <div className="pt-5">
            {count > 0 ? (
              <EducationGrid educations={educations} onOpenDetails={onOpenDetails} />
            ) : (
...
function EducationModal({ education, onClose }) {
  useEffect(() => {
    document.body.style.overflow = education ? "hidden" : "";
    return () => {
    ...
    >
      ...
      >
        <div className="w-full h-48 md:h-56 overflow-hidden" style={{ backgroundColor: THEME.textDark }}>
          <img src={education.image} alt={education.institutionName} className="w-full h-full object-cover" />
        </div>
        ...
        <div className="p-6 md:p-7">
          ...
          <p className="text-sm mb-4" style={{ color: THEME.accent, fontFamily: THEME.fontFamily }}>
            {education.qualification}
            {formatDateRange(education.startDate, education.endDate) ? ` · ${formatDateRange(education.startDate, education.endDate)}` : ""}
          </p>
          ...
          {education.score && (
            <div className="mb-6">
              ...
              <p className="text-sm md:text-base leading-relaxed" style={{ color: THEME.textBody, fontFamily: THEME.fontFamily }}>
                {education.score}
              </p>
            </div>
          )}
...
function ClickHint({ onDismiss }) {
  return (
    ...
    >
      <span>
        <span style={{ color: THEME.accent, fontWeight: 600 }}>Tip:</span> click a category to see the details
      </span>
      <button
...
export default function Education() {
  const [categories, setCategories] = useState(null); // [{ id, label, educations: [...] }, ...] — fully backend-driven
  ...
  return (
    ...
    >
      ...
      {/* Soft green blobs — same treatment as Header/Projects/Experience */}
      <div
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 md:px-12 lg:px-16">
        ...
        {error ? (
          <p className="text-sm" style={{ color: THEME.textMuted, fontFamily: THEME.fontFamily }}>
            Couldn't load education right now — please check back shortly.
          </p>
        ) : categories === null ? (
          <LoadingSkeleton />
        ) : (
            ...
            <div className="flex flex-col gap-4">
              {categories.map((category, index) => (
                <CategorySection
                  ...
                  educations={category.educations}
                  isExpanded={!!expanded[category.id]}
                  onToggle={() => toggleCategory(category.id)}
                  onOpenDetails={setActiveEducation}
                  iconIndex={index}
                />
              ))}
            </div>
      ...
      <EducationModal education={activeEducation} onClose={() => setActiveEducation(null)} />
    </section>
  );
}